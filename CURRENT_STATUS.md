...
- analytics_p1_promotion_status: `[Analytics]` P1 (issue #445: `POPULATION_CONTRACT`, `RECONCILIATION_CONTRACT`) and P1-B (issue #449: strengthened Explanation Challenge, `RECOMMENDATION_EVIDENCE`, stability/persistence check, `FORECAST_METHOD_COMPARISON`, `effect_type`, `what_would_change_the_view`) promoted from bounded pilot to standard/active status, owner-authorized 2026-09-06. Deliberate scope extension beyond issue #451's literal (calibration-only) text, done on the #451 branch/PR at the owner's direction. `ANALYSIS_CONTINUATION_GATE` (§15.3) NOT promoted, remains deferred, verified unchanged. Repository-side promotion only: source text, generated bundles, and provenance were updated and validated (bundles + provenance regenerated and verified, 635/635 tests pass) — repository package ready. `external ChatGPT Project sync: NOT RUN`; `external Analytics behavioral acceptance: NOT RUN` (no live-Project or surrogate-model behavioral test performed; that check is owner-reserved on the real `[Analytics]` Project after upload). Evidence: docs/evidence/ANALYTICS_P1_PROMOTION_2026-09-06.md
