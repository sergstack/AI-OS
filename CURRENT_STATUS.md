...
- native_subagent_dispatch_status: STANDARDIZE BOUNDED (owner-approved direction, conditional); mandatory worktree isolation + structural no-nesting (Plan child cannot use native Agent tool; no write-capable child); commissioning punch-list 4/4 closed — dispatch-evidence schema + blocking linter, enforced telemetry contract, 16 hardened dispatches across 7 owners, guard-calibration proposal (thresholds unchanged), [AI OS] cost/latency owner; flips to STANDARDIZED BOUNDED only on a passing Judge review; not a default/unrestricted standard; evidence in docs/evidence/NATIVE_SUBAGENT_DISPATCH_{PILOT,STANDARDIZATION,COMMISSIONING}_2026-09-02.md + subagent_dispatch_records_2026-09-02.json
