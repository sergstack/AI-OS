...
| `cross_project_eval_review` | evaluate or judge an output, PR, memo, loop, or evidence claim | `[AI OS]` -> owner project | eval type, evidence checked, deterministic checks, judge verdict, required revision, risks, next step | deterministic checks override LLM judge |
...
| `Eval / Judge` | `[AI OS]` -> owner project | output, PR, memo, loop, or evidence claim | eval owner, deterministic checks, judge verdict, revise/block/accept next step | `cross_project_eval_review` |
