...
| `[Codex]` | yes, starts with `# Project Instructions — [Codex]` | 6 bundle files visible: `CODEX_01...06` | `https://chatgpt.com/g/g-p-69f7c5794ab481919143830fc1a513b9-codex/project` |
| `[Inbox Router]` | yes, starts with `# [Inbox / Router] Project Instructions` | 2 bundle files visible: `INBOX_01...02` | `https://chatgpt.com/g/g-p-6a1db92ccf708191aa195f4bf963a0ad-inbox-router/project` |

## Smoke QA Results

The `[AI OS]` result reuses issue #59 evidence from `SMOKE_QA_RESULTS.md`.
Other project smoke QA was run in ChatGPT UI as one combined prompt per project, with one extra isolated rerun for the `[Codex]` Goal Mode question.
...
| `[Codex]` | Goal Mode | Infer safe scope, internal execution package, branch, checks, PR, human review, no auto-merge. | Combined and isolated answers covered broad goal -> internal task package, scope, checks, rollback, acceptance, and blockers, but did not explicitly mention branch, PR, human review, or no auto-merge. | revise | Update or clarify `[Codex]` Goal Mode guidance, then rerun this smoke item. |
| `[Inbox Router]` | Classify 10 raw inputs | Routes each input to one project or unclear with reason and next action; does not solve target work. | Answer returned 10 routing categories and unclear-case handling, not a concrete 10-item raw-input classification. The prompt itself did not provide 10 concrete raw inputs. | revise | Rerun with 10 concrete raw inputs or adjust the smoke prompt to require generated sample inputs. |
...
| `git status --short` | pass | Shows only `CHATGPT_PROJECT_SYNC_CHECKLIST.md` modified and `CROSS_PROJECT_SMOKE_QA_RESULTS.md` untracked before this report is staged. |
...
- `[Codex]`: updated to `done` for observed Project Instructions, Knowledge upload, and smoke QA; pilot remains `not_verified`; Goal Mode revise item recorded.
- `[Inbox Router]`: updated to `done` for observed Project Instructions, Knowledge upload, and smoke QA; pilot remains `not_verified`; classification-prompt revise item recorded.
...
- `[Codex]` Goal Mode answer needs explicit branch / PR / human review / no auto-merge wording.
- `[Inbox Router]` first smoke prompt was underspecified for a concrete 10-item classification run.
...
- UI smoke QA results: `[AI OS]`, `[Thinking]`, `[Analytics]`, `[LLM]` pass; `[Codex]` has one `revise`; `[Inbox Router]` has one `revise`.
...
- Risks / limitations: combined prompts, two revise items, no full byte-for-byte UI instruction comparison, no production promotion.
- Rollback: revert this documentation update and the related `CHATGPT_PROJECT_SYNC_CHECKLIST.md` row updates.
- Acceptance status: `candidate / ready for human review`.

## Next Step

Create follow-up tasks for the `[Codex]` Goal Mode wording and the `[Inbox Router]` 10-item classification smoke prompt, then rerun only those affected smoke items.
