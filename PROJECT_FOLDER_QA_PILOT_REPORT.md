...
Limitations: this review does not prove live ChatGPT Project behavior after manual Project Instructions sync and Knowledge upload. After merge/upload, each project still needs live smoke QA.
...
| `StreamDeck` | 5 | `README.md`; v2.7 setup/map files; learning-cycle candidate | not a ChatGPT Project upload bundle | pass |
...
| StreamDeck | pilot-related Markdown by filename | not found | not applicable | pass | `STREAMDECK_LEARNING_CYCLE_PROMPTS_CANDIDATE.md` mentions pilot readiness but does not match pilot filename patterns from the issue. |
...
The repository-file QA found only fixable pilot metadata/status mismatches. After the minimal fixes above, project roles, routing, governance, output boundaries, handoff behavior, pilot status clarity, blocked promotion constraints, and Codex branch/checks/PR workflow are consistent in the inspected files.
